    ...
    <!--end navigation-->
            ...
            <!--end sidebar wrapper -->
                     ...
                     <header>
                                    ...
                                    <div class="dropdown-menu dropdown-menu-end dropmenu-messages">
                                        <div href="javascript:;">
                                            <div class="msg-header">
                                                ...
                                                <a href="" class="msg-header-clear ms-auto" onclick="markswitcherAllAsRead()">قراءه الكل</a>
                                            </div>
                                        </div>
                                        ...
                                        @forelse($user->unreadNotifications as $notification)
                                            <a class="dropdown-item" href="javascript:;" data-id="{{ $notification->id }}" onclick="markswitcherAsRead(this)">
                      ...
                      <script>
                        function markswitcherAsRead(element) {
                            var id = $(element).data('id');

                            $.ajax({
                                url: '/notification/switcher/read',
                                method: 'POST',
                                data: {
                                    id: id,
                                    _token: '{{ csrf_token() }}'
                   ...
                   <script>
                       function markswitcherAllAsRead() {
                            $.ajax({
                                url: '/notification/switcher/read-all',
                                method: 'POST',
          ...
          {{-- <div class="col">
            <div class="switcher-parent">
                ...
                @foreach ($categories as $item)
                   <a class="active" href="{{$item->id}}"><p>{{ $item->category_name }}</p></a>
                @endforeach
      ...
      <script>
        $(document).ready(function() {
            ...
            function fetchBuildingsByCategory(categoryId) {
                ...
                $.ajax({
                    url: '/fetch-buildings/' + categoryId,
                    type: 'GET',
                    success: function(response) {
                        ...
                        $.each(response.buildings, function(index, item) {
                            ...
                            var row = '<tr>' +
                                ...
                                '<a href="{{ route('show.building', ':id') }}">'.replace(':id', item.id) +
...
<script>
 ...
 @endif
  ...
  function togglePage() {
    ...
    if (currentPage === 1) {
      pageToggle.href = '{{ route('admin.dashboard') }}'; // Set the first page URL
      currentPage = 2;
    } else {
      pageToggle.href = '{{ route('admin.switcher') }}'; // Set the second page URL
      currentPage = 1;
    }
  }
      </script>
